...
import {
    setCustomerDropDown,
    setProductDropDown, setReturnFormValueReset,
    setReturnList,
    setReturnListTotal
} from "../redux/sate-slice/Return-slice.js";
...
export const CreateReturnRequest = async (ParentBody, ChildBody) => {
    try {
        ...
        let URL = `/api/CreateReturn`
        let result = await axios.post(URL, postBody)
        store.dispatch(hideLoader())
        if (result.status === 200 && result.data["status"] === "Success") {
            toast.success("Created Successfully")
            store.dispatch(setReturnFormValueReset())
            return true
        } else {
            toast.error("Something went Wrong")
            return false
